...
> R: Es importante llamarla antes de usar carrito, ya que las variables locales no se inicializan. Si no hacemos eso, cantidad tendria unn valor indeterminado. 
...
> R: Pasa, con cantidad =1, precio por 1 da lo mismo que el precio, por lo que el bug anterior no se nota
...
> R:Falla, esperaba 700 pero le llego 350
...
> R: Deberia multpilicar el precio por la cantidad
...
> R:Falla, y el 5to carrito devuelve 1 en vez de 0, el programa ademas se aborta con "stack smashing detected"
...
> R:El operador incorrecto es el <=, el cual deberia ser unicamente <
...
> R: Las lineas 29 y 30, no se ejecutaron porque make cobertura solo corre los test unitarios, y esos se usan en los de integracion
...
> R: Un test unitario prueba funciones aisladas, mientras que el de integracion prueba como esas funciones trabajan juntas o se integran. En este laboratorio el test unitario lo detecto primero, ya que lo probo de forma aislada
...
> R:En C los arrays no chequean los limites, pudiendo causar crashes o poder ser explotado para ejecutar codigo malicioso. En Python o Java, cada acceso se verifica en tiempo de ejecucion, y si el acceso esta fuera de rango se lanza una excepcion.
...
> R:Si miramos el codigo intentando buscar bugs, es posible que pasemos por alto cosas infimas que puedan romper el codigo, como el = en el <=. En cambio, los test ejecutan el codigo y comparan el resultado esperado con el real, exponiend cualquier bug del codigo.
...
> R: Porque con cantidad = 1, quedaba igual que el precio, asi que el codigo con ese bug quedaba con el resultado que con el codigo correcto. pero con cantidad = 2, ahi si hubo diferencia entre el resultado correcto y el que recibio gracias al bugg.
